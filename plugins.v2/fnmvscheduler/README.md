...
*   **三种扫描模式**：
    ...
    *   **精确扫描**：🆕 指定文件夹扫描，大幅缩短入库时间，无需额外配置。建议网盘挂载到飞牛后映射进mp，而非mp中直接添加网盘。
...
3.  **模式选择**：确认您在扫描规则中设置的模式（本地 vs. 网盘）是否符合实际情况。
4.  **服务器选择**：如果您在配置中选择了特定的媒体服务器，请确保该媒体库属于被选中的服务器。
5.  **精确扫描模式**：如果启用了精确扫描，将无本地/网盘区别，采用同一套处理逻辑。建议网盘挂载到飞牛后映射进mp，而非mp中直接添加网盘。
...
### v2.2.2
*   ✨ **日志优化**: 优化待扫描路径统计，同时显示本库和全部路径数量
*   🧹 **代码清理**: 清理冗余代码，优化项目结构

### v2.2.1
*   🔧 **签名修复**: 修复首次扫描请求签名算法问题，确保一次请求成功
*   🗑️ **逻辑简化**: 去除二次扫描逻辑，删除日志解析相关功能
*   📦 **配置简化**: 移除trim-media.log路径映射要求
...
*   🎯 **签名错误判断**: 实现真正的签名错误判断，区分首次请求失败（继续尝试）和二次请求失败（放弃）
*   🚫 **无回退模式**: 精确扫描模式绝不回退到常规扫描，保持功能纯粹性
*   📱 **通知系统**: 新增精确扫描失败通知功能，支持平台所有通知渠道
